...
def process_detection_task(task_id: str, image_bytes: bytes, content_type: str) -> None:
    repo = SupabaseRepository()
    try:
        repo.update_detection_processing(task_id)
        model_result = detect_image_sync(image_bytes, content_type)
...
def process_detection_task_from_storage(task_id: str, storage_path: str, content_type: str) -> None:
    repo = SupabaseRepository()
    try:
        image_bytes = repo.download_image(storage_path)
        process_detection_task(task_id, image_bytes, content_type)
    except Exception as error:  # noqa: BLE001
        repo.update_detection_failed(task_id, str(error))
