    ...
    <el-card shadow="never">
      <template #header>
        <div class="header-row">
          <h3>检测历史</h3>
          <el-button type="primary" plain @click="$router.push('/stonedirty/detection')">返回检测工作台</el-button>
        </div>
      </template>
      ...
      <el-form inline class="search-row">
        ...
        <el-form-item label="时间范围">
          <el-date-picker v-model="dateRange" type="datetimerange" start-placeholder="开始时间" end-placeholder="结束时间" range-separator="至" value-format="YYYY-MM-DD HH:mm:ss" style="width:360px" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table v-loading="loading" :data="list" border @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template #default="scope">
            <el-tag :type="scope.row.status === 'done' ? 'success' : scope.row.status === 'failed' ? 'danger' : 'warning'">{{ scope.row.status }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="推理模式" width="120"><template #default="scope">{{ getInferenceModeLabel(scope.row.metrics?.inferenceMode) }}</template></el-table-column>
      ...
      <div class="pager-row">
        <el-pagination layout="total, prev, pager, next" :total="total" :page-size="query.size" :current-page="query.currentPage" @current-change="handleCurrentChange" />
      </div>
    </el-card>
    ...
    <el-drawer v-model="detailVisible" title="检测详情" size="50%">
      <template v-if="historyDetailTask">
        <el-descriptions :column="2" border size="small">
          <el-descriptions-item label="任务ID">{{ historyDetailTask.id }}</el-descriptions-item>
          <el-descriptions-item label="图片名称">{{ historyDetailTask.imageName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="状态"><el-tag :type="historyDetailTask.status === 'done' ? 'success' : historyDetailTask.status === 'failed' ? 'danger' : 'warning'" size="small">{{ historyDetailTask.status }}</el-tag></el-descriptions-item>
          <el-descriptions-item label="推理模式">{{ getInferenceModeLabel(historyDetailTask.metrics?.inferenceMode) }}</el-descriptions-item>
          <el-descriptions-item label="建筑">{{ historyDetailTask.buildingName }}</el-descriptions-item>
          <el-descriptions-item label="楼层">{{ historyDetailTask.locationFloor || '-' }}</el-descriptions-item>
          <el-descriptions-item label="分区">{{ historyDetailTask.locationSection || '-' }}</el-descriptions-item>
          <el-descriptions-item label="污渍类型">{{ historyDetailTask.stainType || '-' }}</el-descriptions-item>
          <el-descriptions-item label="污渍占比">{{ historyDetailTask.affectedAreaPercentage ?? '-' }}%</el-descriptions-item>
          <el-descriptions-item label="检测时间" :span="2">{{ historyDetailTask.createdAt || '-' }}</el-descriptions-item>
          <el-descriptions-item label="总结" :span="2">{{ historyDetailTask.summary || '-' }}</el-descriptions-item>
        </el-descriptions>

        <div v-if="historyDetailTask.regions?.length" class="regions-section">
          <p class="section-title">区域明细</p>
          <el-table :data="historyDetailTask.regions" class="regions-table" size="small" stripe border>
            <el-table-column type="index" label="#" />
            <el-table-column prop="label" label="污渍类型" />
            <el-table-column prop="confidence" label="置信度"><template #default="scope">{{ Number(scope.row.confidence).toFixed(4) }}</template></el-table-column>
            <el-table-column label="BBox(x1,y1,x2,y2)"><template #default="scope">{{ Number(scope.row.bbox[0]).toFixed(2) }},{{ Number(scope.row.bbox[1]).toFixed(2) }},{{ Number(scope.row.bbox[2]).toFixed(2) }},{{ Number(scope.row.bbox[3]).toFixed(2) }}</template></el-table-column>
          </el-table>
        </div>

        <el-alert v-if="historyDetailTask.status === 'failed'" type="error" :closable="false" :title="historyDetailTask.errorMessage || '检测失败'" style="margin-top:12px" />

        <div v-if="historyDetailTask.imageSignedUrl || historyDetailTask.processedImageSignedUrl" class="image-section">
          <p class="section-title">检测图像</p>
          <div class="image-grid">
            <div class="image-item">
              <p class="image-label">原图</p>
              <el-image v-if="historyDetailTask.imageSignedUrl" :src="historyDetailTask.imageSignedUrl" :preview-src-list="[historyDetailTask.imageSignedUrl]" fit="contain" style="width:100%;height:300px" preview-teleported />
              <el-empty v-else description="原图不可用" />
            </div>
            <div class="image-item">
              <p class="image-label">检测后图片</p>
              <el-image v-if="historyDetailTask.processedImageSignedUrl" :src="historyDetailTask.processedImageSignedUrl" :preview-src-list="[historyDetailTask.processedImageSignedUrl]" fit="contain" style="width:100%;height:300px" preview-teleported />
...
.regions-section { margin-top:12px }
.regions-table { width:100% }
.image-section { padding-top:12px; border-top:1px solid var(--el-border-color-lighter) }
.section-title { margin:0 0 12px; font-size:14px; font-weight:500 }
.image-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; width:100% }
.image-item { border:1px solid var(--el-border-color-lighter); border-radius:8px; overflow:hidden; background-color:var(--el-fill-color-light); width:100% }
.image-item :deep(.el-image) { width:100%; display:block }
.image-label { margin:8px; font-size:12px; font-weight:500; color:var(--el-text-color-secondary) }
@media (max-width:1024px) { .history-compare-grid { grid-template-columns:1fr } }
</style>
