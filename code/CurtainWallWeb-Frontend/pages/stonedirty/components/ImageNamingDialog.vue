...
function autoFillFromFilename() {
  ...
  if (cleaned && !form.buildingName) {
    form.buildingName = cleaned.substring(0, 50)
  }
  if (!customName.value) {
  ...
  >
    <div class="dialog-content">
      ...
      <el-form label-width="100px" size="small">
        <el-form-item label="图像名称" required>
          <el-input v-model.trim="customName" placeholder="输入有意义的图像名称" maxlength="100" show-word-limit clearable />
          <small class="form-hint">用于识别和管理图像，不影响检测结果</small>
        </el-form-item>

        <el-form-item label="建筑名称" required>
          <el-input v-model.trim="form.buildingName" placeholder="如：A座南立面" maxlength="50" show-word-limit clearable />
        </el-form-item>

        <el-form-item label="楼层">
          <el-input-number v-model="form.locationFloor" :min="1" :max="500" placeholder="可选" clearable />
        </el-form-item>

        <el-form-item label="分区">
          <el-input v-model.trim="form.locationSection" placeholder="如：A-03" maxlength="20" clearable />
        </el-form-item>

        <el-form-item label="备注">
          <el-input v-model.trim="form.description" type="textarea" :rows="3" placeholder="可选，补充采集场景" maxlength="200" show-word-limit />
        </el-form-item>
      </el-form>
    </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" :loading="loading" @click="handleConfirm">添加到队列</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
.image-preview { margin-bottom: 16px; text-align: center; border: 1px dashed var(--el-border-color-light); border-radius: 8px; padding: 12px; background-color: var(--el-fill-color-light) }
.preview-img { max-width: 100%; max-height: 200px; border-radius: 6px }
.form-hint { display: block; margin-top: 4px; color: var(--el-text-color-secondary); font-size: 12px }
